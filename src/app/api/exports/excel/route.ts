...
export async function GET(request:NextRequest){
  const user=await requireAdmin();const periodId=request.nextUrl.searchParams.get("period");if(!periodId)return new Response("Evaluación semestral requerida",{status:400});
  ...
  const wb=new ExcelJS.Workbook();wb.creator="Colegio Franciscano Agustín Gemelli";const summary=wb.addWorksheet("Resumen");summary.addRows([["EVALUACIÓN DOCENTE COLGEMELLI"],["Evaluación semestral",period?.name??""],["Evaluaciones",evaluations?.length??0],["Generado",new Date().toISOString()]]);
